...
IMAGE* img_P1_avatar = nullptr;
IMAGE* img_P2_avatar = nullptr;
...
void flip_atlas(Atlas& src, Atlas& dst)
{
	...
	{
		IMAGE img_flipped;
		flip_image(src.get_img(i), &img_flipped);
...
void load_game_resources()
{
	...
	atlas_sun_explode.load_from_file(_T("resources/sun_explode_%d"), 5);
	...
	mciSendString(_T("open resources/pea_shot_2.mp3 alias pea_shoot_2"), NULL, 0, NULL);
...
int main()
{
	...
	while (true)
	{
		...
		scene_manager.update(delta_tick);
		last_tick_time = current_tick_time;

		cleardevice();
		scene_manager.draw(main_camera);
		FlushBatchDraw();
